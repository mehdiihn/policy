  ...
  const fetchCredits = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        if (data.user.role === "sub-admin") {
          setCredits(typeof data.user.credits === 'number' ? data.user.credits : 0);
        }
      }
    } catch {}
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              {credits === 0 && (
                <div className="mt-2 text-red-600 font-semibold text-md">
                  You are out of credits. Please contact your admin to get more credits.
                </div>
              )}
            </div>
            <Button
